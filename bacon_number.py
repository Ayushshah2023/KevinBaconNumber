...
class BaconNumberCalculator:
                ...
                for actor in actors:
                    ...
                    for other_actor in actors:
                        if actor != other_actor:
                            self.adjList[actor][other_actor] = movie
        pprint(self.adjList)
        # with open(data_file, 'r', encoding=encoding) as file:
        #     for line in file:
        #         movie, *actors = line.strip().split('/')
        #         for actor in actors:
        #             self.actor_to_movies[actor.strip()].add(movie.strip())
        #             self.movie_to_actors[movie.strip()].add(actor.strip())
        #             self.adj_list[actor.strip()].add(movie.strip())
        #             self.adj_list[movie.strip()].add(actor.strip())
    ...
    def calcBaconNumber(self, startActor, endActor):
        ...
        # Method implementation...
        if startActor not in self.adj_list or endActor not in self.adj_list:
            return [-1, []]
        ...
        while queue:
            ...
            if current_actor == endActor:
                return [len(path), path]

            for neighbor in self.adj_list[current_actor]:
                if neighbor not in visited:
                    if current_actor.startswith("movie"):
                        # If current actor is a movie, add it to path
                        queue.append((neighbor, path + [current_actor]))
                    else:
                        # If current actor is an actor, add both movie and actor to path
                        queue.append((neighbor, path + [current_actor, neighbor]))
